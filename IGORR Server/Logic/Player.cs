...
namespace IGORR_Server.Logic
{
    public class Player : GameObject
    {
        ...
        protected Body_Parts.BaseBody _baseBody = new Body_Parts.BaseBody();
        ...
        public Player(Map map, Rectangle spawnPos, int id)
        ...
        public Player(Map map, string CharFile, Rectangle spawnPos, int id):base(map,spawnPos,id)
        {
            _groupID = 1;
            _charFile = CharFile;
        ...
        public virtual void Update(Map map, float seconds)
        {
            _map = map;
            if (_moveVector.Y != 0)
            {
            ...
            if (wallCollision)
            {
                ...
                TryMove(seconds * _speed.Y * Vector2.UnitY, map);
            }
        ...
        public void Knockback(Vector2 move)
        {
            ...
            airstun = true;
            _onGround = false;
            IGORRProtocol.Messages.KnockbackMessage kbm = (IGORRProtocol.Messages.KnockbackMessage) IGORRProtocol.Protocol.NewMessage(IGORRProtocol.MessageTypes.Knockback);
            kbm.id = this.ID;
            kbm.Move = move;
            kbm.Encode();
        ...
        private void LevelUp()
        {
            ...
            if (_baseBody.Level % 2 == 0)
            {
                ...
                IGORRProtocol.Messages.PlayerInfoMessage pim = (IGORRProtocol.Messages.PlayerInfoMessage)IGORRProtocol.Protocol.NewMessage(IGORRProtocol.MessageTypes.PlayerInfoMessage);
